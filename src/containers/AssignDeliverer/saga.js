...
import { BUSINESS_ORDERS_API, ORDERS_LIST_DELIVERER_API } from "../../../utils/api";
...
export function* getOrdersFunc(action) {
  try {
    ...
    } = yield call(request, BUSINESS_ORDERS_API("food", page, ORDERS_PAGE_SIZE), { domain }, "GET");
...
export function* setDeliverers(action) {
  try {
    ...
    if (action.data.deliverer || action.data.deliverer === "") {
      const {
        response: { data },
      } = yield call(
        request,
      ...
      if (data) {
        yield put(getFoodAdminOrders(action.data.page));
